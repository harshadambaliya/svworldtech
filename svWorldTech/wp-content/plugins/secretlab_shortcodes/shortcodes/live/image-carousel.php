...
var output = '',
    ...
    custom_img_size = ( atts['custom_img_size'] !== undefined ) ? atts['custom_img_size'] : '400x200';
...
if( typeof(images) == 'object' && images !== '' ) {
	for ( var i=0; i< images.length; i++ ){
        ...
        if( 'custom_size' == img_size ){
            if( custom_img_size.indexOf('x') > 0 ){
                attachment_data.push( ajaxurl + '?action=kc_get_thumbn_size&id=' + image_id + '&size=' + custom_img_size );
            }else{
                attachment_data.push( ajaxurl + '?action=kc_get_thumbn&id=' + image_id + '&size=' + custom_img_size );
            }
        } else {
	...
	el_classes.push( 'owl-nav-arrow' );
	el_classes.push( 'kc-sync1' );
    el_classes.push( 'ssc-carousel-main-img' );
	...
	#>
	<div class="kc-sync2 owl-carousel ssc-carousel-tumb-imgs">
		<# 
